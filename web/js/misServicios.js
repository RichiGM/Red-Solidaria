...
document.getElementById('editServiceForm').addEventListener('submit', async function (event) {
    ...
    const servicio = {
        idServicio: parseInt(document.getElementById('editServiceId').value),
...
async function cargarServicios() {
    try {
        const idUsuario = await obtenerIdUsuarioPorEmail(emailLocal);
        const response = await fetch(`http://localhost:8080/RedSolidaria/api/servicio/mis-servicios?idUsuario=${idUsuario}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`
            }
