function loadHeaderAndFooter() {
  ...
  fetch("header.html")
    ...
    .then((data) => {
      ...
      // Agregar enlace al Banco de Tiempo
      addTimeBankLink();
    })
...
// Función para agregar enlace al Banco de Tiempo
function addTimeBankLink() {
  const userMenu = document.querySelector(".user-menu");
  if (userMenu) {
    // Crear enlace al Banco de Tiempo
    const timeBankLink = document.createElement("li");
    timeBankLink.innerHTML = '<a href="bancoTiempo.html" id="bancoTiempo">Banco de Tiempo</a>';

    // Insertar después de "Mis Servicios"
    const misServiciosItem = document.getElementById("misServicios");
    if (misServiciosItem && misServiciosItem.parentNode) {
      misServiciosItem.parentNode.insertBefore(timeBankLink, misServiciosItem.nextSibling);
    } else {
      userMenu.appendChild(timeBankLink);
    }
  }
}
